...
def analyze_cashback_by_category(
    ...
    """
    logger.info(f"Анализ кешбэка по категориям: {year}-{month:02d}, количество транзакций: {len(data)}")
...
def investment_bank(month: str, transactions: List[Dict[str, Any]], limit: Any) -> str:
    """
    ...
    :param transactions: Список словарей с транзакциями.
        ...
        - 'Категория': str
    :param limit: Порог округления — 10, 50 или 100 рублей (может быть int или float).
    :return: JSON-строка в формате {"total_savings": X.XX}
    """
    # Логируем начало и тип limit для отладки
    logger.info(f"Расчёт Инвесткопилки для месяца: {month}, лимит: {limit} (тип: {type(limit)})")

    # Приводим limit к целому числу, если возможно
    try:
        limit = int(limit)
    except (TypeError, ValueError):
        logger.error(f"Некорректный лимит (не число): {limit}, тип: {type(limit)}")
    ...
    try:
        ...
        if not (2018 <= year <= 2021) or not (1 <= month_num <= 12):
            logger.warning(f"Месяц вне диапазона: {month}")
            return json.dumps({"total_savings": 0.0}, ensure_ascii=False, indent=2)
    ...
    # Возвращаем JSON
    return json.dumps({"total_savings": total_savings}, ensure_ascii=False, indent=2)
...
def search_person_transfers() -> str:
    ...
    Условия:
      - Категория: «Переводы»
      - В описании: имя + пробел + заглавная буква кириллицы + точка (например: «Валерий А.»)
    ...
    try:
        transactions: list[dict[str, Any]] = read_transactions_from_excel(str(TRANSACTIONS_FILE))
    ...
    for transaction in transactions:
        ...
        # Проверяем на 'nan', 'none' и пустоту после преобразования
        if category.lower() in ("nan", "none", "") or description.lower() in ("nan", "none", ""):
            continue
...
#     result = investment_bank(month="2021-03", transactions=transactions, limit=50)
